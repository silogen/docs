...
To facilitate security and isolation of workloads, each project triggers the creation of a corresponding namespace in the underlying Kubernetes cluster. This namespace is used to contain all resources related to that project, ensuring that workloads and data are properly segregated. Users assigned to a project are granted access to its namespace, allowing them to manage and run workloads within that specific context.
