...
We currently support uploading datasets in a JSONL format, where each row represents a separate chat conversation. The format of each row should correspond to what is defined in our [specification](datasets.mdx#fine-tuning) for each datapoint.
...
You can view the details of the merge process in the "Fine-tuning status" section of the page
